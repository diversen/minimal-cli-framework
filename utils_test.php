...
echo $utils->colorOutput("Exectuing a command that that properbly exists (dir)\n");
$res = $utils->execCommand('dir');
echo $utils->colorOutput("$res is the result of the above operation\n", 'light_blue');

echo $utils->colorOutput("Exectuing a command that that properbly exists (dir) - but without status and command messages\n");
$res = $utils->execCommand('dir', 0, 0);
echo $utils->colorOutput("$res is the result of the above operation\n", 'light_blue');
...
echo $utils->colorOutput("STATUS ", 'light_green', 'A status about how we are doing it');

echo $utils->colorOutput("Prompt the user for input\n");
